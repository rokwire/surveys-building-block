...
func TestApplication_GetEnvConfigs(t *testing.T) {
	...
	storage := mocks.NewStorage(t)
	storage.On("FindConfig", model.ConfigTypeEnv, rokwireutils.AllApps, rokwireutils.AllOrgs).Return(&config, nil)
	app := buildTestApplication(storage)

	storage2 := mocks.NewStorage(t)
	storage2.On("FindConfig", model.ConfigTypeEnv, rokwireutils.AllApps, rokwireutils.AllOrgs).Return(nil, errors.New("no config found"))
